...
**核心原则**：本技能不涉及任何代码编写，只产出文档。所有文档经过人类多轮评审定稿后，才进入编码阶段。**例外**：设计师在阶段三可临时创建实验性代码（位于 `docs/modules/{NN}-{name}/tmp/` 目录）用于推演接口可行性或验证数据流，用完即弃，不进入版本管理（详见 phase-3.md §10）。
...
| `AGENTS.md` | 项目级工程规则与约束的入口指引。包含文件/目录权限、提交规范、测试要求、升级条件、跨模块规则、决策记录机制、模块目录说明等。由 OpenSDD 各阶段增量积累，人类最终锁定。**存在时各阶段 AI 角色均自动加载** | 所有角色（存在时） |
