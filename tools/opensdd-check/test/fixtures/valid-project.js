...
function createValidProject(overrides = {}) {
  ...
  const moduleRows = moduleDirs.map((m) => {
    const parts = m.split('-');
    const num = parts[0];
  ...
  fs.writeFileSync(path.join(docsDir, 'PLAN.md'), [
    '# 任务计划',
    ...moduleDirs.map((m, i) => `- [ ] T-${String(i + 1).padStart(3, '0')}: 实现接口 [${m}/DESIGN.md#${m.split('-')[0]}-F001]`),
  ].join('\n'), 'utf-8');
  ...
  for (const m of moduleDirs) {
    ...
    fs.writeFileSync(path.join(modPath, 'API.md'), [
      `# ${m} 模块接口`,
      '## 模块概述与职责边界',
      '职责。',
      '## 核心数据结构',
    ...
    fs.writeFileSync(path.join(modPath, 'DESIGN.md'), [
      `# ${m} 模块内部实现`,
      '## 核心逻辑流程',
      '流程。',
      '## 内部实现细节',
