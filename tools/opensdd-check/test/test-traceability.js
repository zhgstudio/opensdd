...
describe('TRACEABILITY', () => {
  ...
  function createProjectStructure(specReqs, featuresByModule) {
    ...
    const specContent = [
      '# SPEC',
      ...specReqs.map((id) => `- ${id}: some requirement`),
    ].join('\n');
  ...
  it('should pass when REQs and features both exist', async () => {
    createProjectStructure(['REQ-001', 'REQ-002'], { '01-auth': ['01-F001', '01-F002'] });
    const result = await check(tmpDir);
  ...
  it('should warn when REQs exist but no features', async () => {
    ...
    assert.ok(result.messages[0].includes('2 requirement(s)'));
    assert.ok(result.messages[0].includes('no features'));
  });
